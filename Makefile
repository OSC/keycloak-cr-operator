...
setup-test-e2e: ## Set up a Kind cluster for e2e tests if it does not exist
	...
	@case "$$($(KIND) get clusters)" in \
		...
		*) \
			...
			$(KIND) create cluster --config test/e2e/kind.yaml --name $(KIND_CLUSTER) ;; \
	esac
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name keycloak-cr-operator-builder
	$(CONTAINER_TOOL) buildx use keycloak-cr-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm keycloak-cr-operator-builder
	rm Dockerfile.cross
